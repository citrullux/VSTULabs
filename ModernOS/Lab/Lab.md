...
Как можно видеть по символу `l` в моём случае это символьная ссылка, которая не отображает свойств самого файла. Потому изменим команду на `ls -lai /usr/bin/dash`

`1357 -rwxr-xr-x 1 root root 125688 Mar 23  2022 /usr/bin/dash`

И как можно увидеть файл sh принадлежит пользователю root и группе root. Но для всех пользователей операционной системы файл доступен на чтение и исполнение, что приводит к тому, что sh можно исполнить, а приложение `sh` само выполнит написанный вами файл как захочет, права на чтение файла `main.sh` у вас есть.